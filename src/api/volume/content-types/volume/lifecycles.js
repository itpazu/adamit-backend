...
module.exports = {
  async beforeUpdate(event) {
    ...
    // if "unpublishing" or saving unpublished volume simply return
    ...
    if (data.publishedAt) {
      ...
      if (!volumeResponse.mainMedia[0]?.gallery) {
        error.message =
          "you need to provide at least one gallery before publishing";
        throw error;
      }
    }
    //if saving data
      ...
      // VALIDATION for Gallery component 0 must have at least one gallery
      // necessary in order to get gallery component data
      const ctx = strapi.requestContext.get().request.body;

      if (ctx?.mainMedia[0]?.gallery) {
        validateArticlesAmount(
          volumeResponse.mainMedia[0]?.gallery,
          ctx.mainMedia[0].gallery,
          "gallery",
          1,
          error
        );
